...
class RouterHandlerCore {
    constructor(store, platformAdapter) {
        this.store = store
        this.platformAdapter = platformAdapter // 平台适配器，处理平台特定操作
    ...
    async execute(routeCommand) {
        ...
        if (typeof window !== 'undefined') {
            window.__ROUTER_HANDLER_EXECUTING__ = true
        }
        ...
        try {
            ...
            // 清除执行标志
            if (typeof window !== 'undefined') {
                window.__ROUTER_HANDLER_EXECUTING__ = false
            }
...
export default RouterHandlerCore
export { ROUTE_COMMAND_VERSION }